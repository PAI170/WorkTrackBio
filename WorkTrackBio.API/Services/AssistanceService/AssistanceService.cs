...
namespace WorkTrackBio.API.Services.AssistanceService
{
    public class AssistanceService : IAssistanceService
    {
        ...
        public async Task<ApiResponse<IEnumerable<AssistanceDataTransferObject>>> GetAllAssistancesAsync()
        {
            ...
            return ApiResponse<IEnumerable<AssistanceDataTransferObject>>.SuccessResponse(assistanceDtos, $"Se encontraron {assistanceDtos.Count()} registro(s) de asistencia");
        }

        public async Task<ApiResponse<AssistanceDataTransferObject>> GetAssistanceByIdAsync(int id)
        {
            if (id <= 0)
                return ApiResponse<AssistanceDataTransferObject>.ErrorResponse("El ID debe ser mayor que 0", 400);

            var assistance = await _assistanceRepository.GetByIdAsync(id);
            if (assistance == null)
                return ApiResponse<AssistanceDataTransferObject>.ErrorResponse($"No se encontró un registro de asistencia con ID {id}", 404);

            var assistanceDto = _mapper.Map<AssistanceDataTransferObject>(assistance);

            return ApiResponse<AssistanceDataTransferObject>.SuccessResponse(assistanceDto,"Registro de asistencia encontrado exitosamente");
        }

        public async Task<ApiResponse<IEnumerable<AssistanceDataTransferObject>>> GetAssistancesByEmployeeAsync(int employeeId)
        {
            if (employeeId <= 0)
                return ApiResponse<IEnumerable<AssistanceDataTransferObject>>.ErrorResponse("El ID del empleado debe ser mayor que 0", 400);

            var assistances = await _assistanceRepository.GetByEmployeeAsync(employeeId);
            var assistanceDtos = _mapper.Map<IEnumerable<AssistanceDataTransferObject>>(assistances);

            return ApiResponse<IEnumerable<AssistanceDataTransferObject>>.SuccessResponse(assistanceDtos, $"Se encontraron {assistanceDtos.Count()} registro(s) de asistencia para el empleado {employeeId}");
        }

        public async Task<ApiResponse<IEnumerable<AssistanceDataTransferObject>>> GetAssistancesByProjectAsync(int projectId)
        {
            if (projectId <= 0)
                return ApiResponse<IEnumerable<AssistanceDataTransferObject>>.ErrorResponse("El ID del proyecto debe ser mayor que 0", 400);

            var assistances = await _assistanceRepository.GetByProjectAsync(projectId);
            var assistanceDtos = _mapper.Map<IEnumerable<AssistanceDataTransferObject>>(assistances);

            return ApiResponse<IEnumerable<AssistanceDataTransferObject>>.SuccessResponse(assistanceDtos,  $"Se encontraron {assistanceDtos.Count()} registro(s) de asistencia para el proyecto {projectId}");
        }
        ...
        public async Task<ApiResponse<IEnumerable<AssistanceDataTransferObject>>> GetAssistancesByDateAsync(DateOnly date)
        {
            ...
            return ApiResponse<IEnumerable<AssistanceDataTransferObject>>.SuccessResponse(assistanceDtos,  $"Se encontraron {assistanceDtos.Count()} registro(s) de asistencia para la fecha {date:dd/MM/yyyy}");
        }
        ...
        public async Task<ApiResponse<IEnumerable<AssistanceDataTransferObject>>> GetAssistancesByDateRangeAsync(DateOnly startDate, DateOnly endDate)
        {
            ...
            if (startDate > endDate)
                return ApiResponse<IEnumerable<AssistanceDataTransferObject>>.ErrorResponse("La fecha de inicio no puede ser posterior a la fecha de fin", 400);

            var assistances = await _assistanceRepository.GetByDateRangeAsync(startDate, endDate);
            var assistanceDtos = _mapper.Map<IEnumerable<AssistanceDataTransferObject>>(assistances);

            return ApiResponse<IEnumerable<AssistanceDataTransferObject>>.SuccessResponse(assistanceDtos, $"Se encontraron {assistanceDtos.Count()} registro(s) de asistencia entre {startDate:dd/MM/yyyy} y {endDate:dd/MM/yyyy}");
        }
        ...
        public async Task<ApiResponse<IEnumerable<AssistanceDataTransferObject>>> GetAssistancesByEmployeeAndProjectAsync(int employeeId, int projectId)
        {
            ...
            return ApiResponse<IEnumerable<AssistanceDataTransferObject>>.SuccessResponse(assistanceDtos, $"Se encontraron {assistanceDtos.Count()} registro(s) de asistencia para el empleado {employeeId} en el proyecto {projectId}");
        }
        ...
        public async Task<ApiResponse<IEnumerable<AssistanceDataTransferObject>>> GetAssistancesByEmployeeAndDateRangeAsync(int employeeId, DateOnly startDate, DateOnly endDate)
        {
            ...
            return ApiResponse<IEnumerable<AssistanceDataTransferObject>>.SuccessResponse(assistanceDtos,  $"Se encontraron {assistanceDtos.Count()} registro(s) de asistencia para el empleado {employeeId} entre {startDate:dd/MM/yyyy} y {endDate:dd/MM/yyyy}");
        }
        ...
        public async Task<ApiResponse<IEnumerable<AssistanceDataTransferObject>>> GetAssistancesByProjectAndDateRangeAsync(int projectId, DateOnly startDate, DateOnly endDate)
        {
            ...
            return ApiResponse<IEnumerable<AssistanceDataTransferObject>>.SuccessResponse(assistanceDtos, $"Se encontraron {assistanceDtos.Count()} registro(s) de asistencia para el proyecto {projectId} entre {startDate:dd/MM/yyyy} y {endDate:dd/MM/yyyy}");
        }
        ...
        public async Task<ApiResponse<IEnumerable<AssistanceDataTransferObject>>> GetAssistancesByProjectAndEmployeeAsync(int projectId, int employeeId)
        {
            ...
            return ApiResponse<IEnumerable<AssistanceDataTransferObject>>.SuccessResponse(assistanceDtos, $"Se encontraron {assistanceDtos.Count()} registro(s) de asistencia para el proyecto {projectId} y empleado {employeeId}");
        }
        ...
        public async Task<ApiResponse<AssistanceDataTransferObject>> CreateAssistanceAsync(CreateAssistanceDataTransferObject createDto)
        {
            ...
            return ApiResponse<AssistanceDataTransferObject>.SuccessResponse(assistanceDto,"Registro de asistencia creado exitosamente", 201);
        }
        ...
        public async Task<ApiResponse<AssistanceDataTransferObject>> UpdateAssistanceAsync(int id, UpdateAssistanceDataTransferObject updateDto)
        {
            ...
            if (!hasChanges)
            {
                return ApiResponse<AssistanceDataTransferObject>.ErrorResponse("No se detectaron cambios para actualizar", 404);
            }
            ...
            return ApiResponse<AssistanceDataTransferObject>.SuccessResponse(assistanceDto, "Registro de asistencia actualizado exitosamente");
        }
        ...
        public async Task<ApiResponse<bool>> DeleteAssistanceAsync(int id)
        {
            ...
            var existingAssistance = await _assistanceRepository.GetByIdAsync(id);
            if (existingAssistance == null)
                return ApiResponse<bool>.ErrorResponse($"No se encontró un registro de asistencia con ID {id}", 404);

            var hasDependencies = await _assistanceRepository.HasDependenciesAsync(id);
            if (hasDependencies)
                return ApiResponse<bool>.ErrorResponse("No se puede eliminar el registro de asistencia porque tiene dependencias (registros de auditoría)", 500);

            var deleted = await _assistanceRepository.DeleteAsync(id);
            if (!deleted)
                return ApiResponse<bool>.ErrorResponse("Error al eliminar el registro de asistencia", 500);

            return ApiResponse<bool>.SuccessResponse(true,"Registro de asistencia eliminado exitosamente");
        }
        ...
        public async Task<ApiResponse<decimal>> CalculateTotalHoursAsync(int assistanceId)
        {
            ...
            return ApiResponse<decimal>.SuccessResponse(totalHours, $"Horas totales calculadas: {totalHours} horas");
        }
    }
}
